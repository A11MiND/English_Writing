...
def upgrade() -> None:
    ...
    op.execute(
        sa.text(
            ...
            """
        ).bindparams(id=SCHOOL_ID, name="Demo Primary School", code="DEMO")
    )
    users = [
        (SYSTEM_ADMIN_ID, None, "system@school.example", "System Administrator", "SYSTEM_ADMIN", "ACTIVE"),
        (SCHOOL_ADMIN_ID, SCHOOL_ID, "admin@school.example", "School Administrator", "SCHOOL_ADMIN", "ACTIVE"),
        (TEACHER_ID, SCHOOL_ID, "teacher@school.example", "English Teacher", "TEACHER", "ACTIVE"),
        (STUDENT_ID, SCHOOL_ID, "student@school.example", "P5A Student", "STUDENT", "ACTIVE"),
        (SUSPENDED_ID, SCHOOL_ID, "suspended@school.example", "Suspended Student", "STUDENT", "SUSPENDED"),
        (ARCHIVED_ID, SCHOOL_ID, "archived@school.example", "Archived Teacher", "TEACHER", "ARCHIVED"),
    ]
    for user in users:
        op.execute(
            sa.text(
