...
def test_admin_import_rejects_duplicate_email() -> None:
    ...
    response = session.post(
        ...
        json={
            ...
            "rows": [
                {
                    "email": "student@wfjosephlee.edu.hk",
...
def test_admin_imports_valid_student() -> None:
    ...
    response = session.post(
        ...
        json={
            ...
            "rows": [
                {
                    "email": f"student.{suffix}@wfjosephlee.edu.hk",
...
def test_admin_import_can_use_external_user_id_for_openauth_subject_mapping() -> None:
            ...
            "rows": [
                {
                    ...
                    "email": f"student.external.{suffix}@wfjosephlee.edu.hk",
    ...
    imported = next(
        ...
        if row["email"] == f"student.external.{suffix}@wfjosephlee.edu.hk"
    )
...
def test_admin_can_suspend_and_restore_account_status() -> None:
    ...
    student = next(
        row for row in users_response.json()["data"]["users"] if row["email"] == "student@wfjosephlee.edu.hk"
    )
